...
class AdminController extends Controller
{
   public function adminDashboard(Request $request)
    ...
    public function generalSettings(Request $request)
    {
     ...
     public function updateLightModeLogo(Request $request)
    {
        $settings = GeneralSetting::take(1)->first();
        if (!is_null($settings)) {
            $path = 'images/site/';
            $old_logo = $settings->site_light_mode_logo;
            $file = $request->file('site_light_mode_logo');
            $filename = 'logo_' . uniqid() . '.png';

            if ($request->hasFile('site_light_mode_logo')) {
                ...
                if ($upload) {
                    ...
                    $settings->update(['site_light_mode_logo' => $filename]);
     ...
     public function updateDarkModeLogo(Request $request)
    {
        $settings = GeneralSetting::take(1)->first();
        if (!is_null($settings)) {
            $path = 'images/site/';
            $old_logo = $settings->site_dark_mode_logo;
            $file = $request->file('site_dark_mode_logo');
            $filename = 'logo_' . uniqid() . '.png';

            if ($request->hasFile('site_dark_mode_logo')) {
                $upload = $file->move(public_path($path), $filename);
                ...
                if ($upload) {
                    ...
                    $settings->update(['site_dark_mode_logo' => $filename]);
                    return response()->json(['status' => 1, 'image_path' => $path . $filename, 'message' => 'Site logo has been updated successfully']);
                } else {
                    return response()->json(['status' => 0, 'message' => 'Something went wrong in uploading new logo']);
                }
